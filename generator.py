...
def expand_keywords(niche: str) -> Tuple[List[str], List[str]]:
    """Very lightweight keyword expander.
    - Primary: full phrase + up to 3 significant tokens.
    - Secondary: unique word list + a few product terms.
    """
    base = niche.strip()
    words = [w for w in re.split(r"[^a-z0-9]+", base.lower()) if w]
    uniq = list(dict.fromkeys(words))
    primary = [base] + uniq[:3]
    secondary = uniq + [f"{w} shirt" for w in uniq[:3]] + ["gift", "present", "apparel", "tee", "t-shirt"]
...
# --- Module-scope helpers for providers ---
...
def generate_images_a1111(sd_url: str, concepts: List[Concept], niche: str, steps: int = 28, cfg: float = 6.5, width: int = 768, height: int = 768) -> None:
    ...
    for idx, c in enumerate(concepts, start=1):
        try:
            ...
            print(f"[warn] a1111 image generation failed for #{idx}: {e}")
...
def generate_images_openai(concepts: List[Concept], niche: str, model: str = "gpt-image-1", size: str = "1024x1024") -> None:
    ...
    for idx, c in enumerate(concepts, start=1):
        ...
        for api_key in keys:
            try:
                p = _generate_image_openai_single(c.image_prompt, os.path.join(img_dir, f"img_{idx:02d}.png"), model, size, api_key)
                c.image_path = p
                last_err = None
                break
            except Exception as e:
...
def generate_images_stability(concepts: List[Concept], niche: str, engine: str = "stable-diffusion-xl-1024-v1-0", steps: int = 30, cfg: float = 7.0, width: int = 1024, height: int = 1024) -> None:
    """Generate images via Stability AI REST API.
    Requires STABLE_DIFFUSION_API_KEY in env (Stability API key).
    Docs: https://platform.stability.ai/docs/api-reference#tag/v1generation
    """
    api_key = _os.environ.get("STABLE_DIFFUSION_API_KEY")
    if not api_key:
        raise RuntimeError("STABLE_DIFFUSION_API_KEY is not set in environment")
    ...
    for idx, c in enumerate(concepts, start=1):
        try:
            payload = {
                "text_prompts": [{"text": c.image_prompt, "weight": 1}],
                "cfg_scale": cfg,
                "width": width,
                "height": height,
                "steps": steps,
            ...
            if not r.ok:
                ...
                raise RuntimeError(f"Stability HTTP {r.status_code}: {msg}")
            data = r.json()
            # Expect: { artifacts: [ { base64: "...", seed: ..., finishReason: ... } ] }
            ...
            print(f"[warn] stability image generation failed for #{idx}: {e}")

# --- Providers: Google Gemini/Imagen ---
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    gm = genai.GenerativeModel(model_name=resolved_model)
                resp = gm.generate_content(c.image_prompt, generation_config={"response_mime_type": "image/png"})
                b64 = _extract_inline_b64_from_gemini_response_obj(resp)
                if not b64:
                    raise RuntimeError("Gemini SDK returned no inline image data")
                out_path = os.path.join(img_dir, f"img_{idx:02d}.png")
                with open(out_path, 'wb') as f:
                ...
                last_err = sdk_err  # noqa: F841

            # Fallback to HTTP (may require OAuth for Imagen models)
            base_url = f"https://generativelanguage.googleapis.com/v1beta/models/{resolved_model}:generateContent?key={api_key}"
            payload = {
            ...
            # Show concise message, typical 401 indicates OAuth is required for Imagen
        ...
        for provider in provider_order:
            try:
                if provider == 'openai' and OPENAI_KEY:
                    ...
                    r = requests.post(f"{args.sd_url.rstrip('/')}/sdapi/v1/txt2img", json=payload, timeout=180)
                    r.raise_for_status()
                    data = r.json()
                    ...
                    try:
                        gm = getattr(args, 'gemini_model', 'imagen-3.0')
                        # Reuse the single-shot helper via public functions
                        _os.environ['GEMINI_API_KEY'] = GEMINI_KEY
                        generate_images_gemini([c], niche, model=gm)
                        if c.image_path:
                            success = True
                            break
                        else:
                    ...
                    try:
                        generate_images_stability([c], niche, engine=getattr(args, 'stability_engine', 'stable-diffusion-xl-1024-v1-0'), steps=getattr(args, 'steps', 30), cfg=getattr(args, 'cfg', 7.0), width=getattr(args, 'width', 1024), height=getattr(args, 'height', 1024))
                        if c.image_path:
                            success = True
                            break
                        else:
                            raise RuntimeError('Stability did not return an image')
                    except Exception as e:
                        last_err = e
                        continue
                else:
...
def main():
    ...
    parser.add_argument("--provider", type=str, default="a1111", choices=["a1111","openai","replicate","gemini","stability"], help="Image generation provider")
    ...
    parser.add_argument("--stability-engine", type=str, default="stable-diffusion-xl-1024-v1-0", help="Stability AI engine id (provider=stability)")
    ...
    if args.images:
        ...
        if provider_order:
            ...
            if args.provider == "a1111":
                ...
                generate_images_stability(concepts, niche, engine=args.stability_engine, steps=args.steps, cfg=args.cfg, width=args.width, height=args.height)
